...
#define RX_FIFO_SIZE (4 * _PAGE_SIZE)
...
typedef struct _ttyacm_t {
	...
	usb_endpoint_desc_t ep_in, ep_out, ep_intr;
	int error;
...
int _telit_write(ttyacm_t *acm, char *data, size_t size)
{
	...
	memset(&urb.setup, 0, sizeof(usb_setup_packet_t));
	...
	if (acm->error || hostproxy_write(&urb, data, size) < 0) {
		TRACE_FAIL("write");
		_telit_reset();
		err = -EIO;
	}
...
static void telit_msgThread(void *arg)
{
	...
	for (;;) {
		...
		case mtGetAttr:
			if ((id < 0) || (id > sizeof(telit_common.data) / sizeof(telit_common.data[0])) || (msg->i.attr.type != atPollStatus)) {
...
int open_pipe(usb_endpoint_desc_t *desc)
{
	usb_open_t open = { 0 };
...
int acm_control(int type, int request, int value, int index, void *buffer, int length)
{
	...
	if (urb.direction == usb_transfer_out)
		return hostproxy_write(&urb, buffer, length);
	else
		return hostproxy_read(&urb, buffer, length);
}


int open_ttyacm(ttyacm_t *acm, int bulk_iface, int intr_iface, usb_endpoint_desc_t *inep, usb_endpoint_desc_t *outep, usb_endpoint_desc_t *intrep)
{
...
int _telit_init_read_buffers(ttyacm_t *acm)
{
	...
	while (acm->read_buffers < 8) {
		if (hostproxy_read(&urb, NULL, 0) < 0) {
			err = -EIO;
			break;
		}
		acm->read_buffers++;
...
usb_endpoint_desc_t *find_endpoint(int address)
{
	struct usb_desc_header *d = (void *)telit_common.conf_descriptor;
	void *end = (char *)d + telit_common.conf_descriptor->wTotalLength;
	usb_endpoint_desc_t *e;

	while ((void *)d < end) {
		if (d->bDescriptorType == USB_DESC_ENDPOINT) {
			e = (usb_endpoint_desc_t *)d;
...
int telit_init_device(void)
{
	...
	if (acm_control(type, REQ_SET_CONFIGURATION, 1, 0, NULL, 0) < 0) {
...
int telit_init_all(void)
{
	...
	int err;
	usb_endpoint_desc_t *inep, *outep, *intrep;
	int bulk_interface, intr_interface;
...
int _telit_reset(void)
{
	...
	if (hostproxy_reset(telit_common.device_id) < 0)
		return -1;
...
static void clear_halt(usb_endpoint_desc_t ep)
{
	acm_control(REQUEST_TYPE_STANDARD | REQUEST_DIR_HOST2DEV | REQUEST_RECIPIENT_ENDPOINT,
		REQ_CLEAR_FEATURE, USB_ENDPOINT_HALT, ep.bEndpointAddress, NULL, 0);
}
...
void event_cb(usb_event_t *usb_event, char *data, size_t size)
{
	...
	case usb_event_insertion:
		...
		memcpy(&telit_common.dev_descriptor, &usb_event->insertion.descriptor, sizeof(usb_device_desc_t));
...
void telit_monitorthr(void *arg)
{
	...
	for (;;) {
		...
		acm_control(type, REQ_GET_STATUS, 0, acm->ep_in.bEndpointAddress, &in_status, 2);
		acm_control(type, REQ_GET_STATUS, 0, acm->ep_out.bEndpointAddress, &out_status, 2);
...
int usb_connect(void)
{
	...
	if (hostproxy_connect(&id, (void *)event_cb)) {
...
int main(int argc, char **argv)
{
	...
	TRACE("Started gsm, log 2");
	if (hostproxy_init() < 0) {
		TRACE_FAIL("init libusb");
		return -1;
	}

	TRACE("Started gsm, log 2a");
	...
	TRACE("Started gsm, log 3");
	if (usb_connect() < 0)
		return -1;

	TRACE("Started gsm, log 4");
