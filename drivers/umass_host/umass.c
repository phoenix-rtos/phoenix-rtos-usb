...
void event_cb(usb_event_t *usb_event, char *data, size_t size)
{
	...
	case usb_event_insertion:
		...
		umass_common.device_id = usb_event->device_id;
		memcpy(&umass_common.dev_descriptor, &usb_event->insertion.descriptor, sizeof(usb_device_desc_t));
		umass_common.conf_descriptor = malloc(size);
		memcpy(umass_common.conf_descriptor, data, size);
		mutexUnlock(umass_common.lock);
		hostproxy_dumpConfiguration(stdout, umass_common.conf_descriptor);
...
int umass_connect(void)
{
	...
	if (hostproxy_connect(&usb_device_id, (void *)event_cb))
		return -1;
...
int umass_open_endpoints(void)
{
	...
	umass_common.in_pipe = hostproxy_open(&open);
	...
	umass_common.out_pipe = hostproxy_open(&open);
...
int umass_transmit_out(void *buffer, int size)
{
	...
	return hostproxy_write(&urb, buffer, size);
}
...
int umass_transmit_in(void *buffer, int size)
{
	...
	return hostproxy_read(&urb, buffer, size);
}


int umass_transmit_cfg(usb_setup_packet_t setup, void *buffer, int size, int direction)
{
	usb_urb_t urb = {
	...
	return direction == usb_transfer_in ? hostproxy_read(&urb, buffer, size) : hostproxy_write(&urb, buffer, size);
}
